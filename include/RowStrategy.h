...
class RowStrategy final : public iBotStrategy {
    ...
    std::optional<std::vector<std::pair<int, int>>> get_next_win_state(const std::vector<std::pair<int, int>> &cur_state) noexcept;
    std::optional<std::vector<int>> make_transition(const std::vector<bool> &from, const std::vector<std::pair<int, int>> &to) noexcept;
